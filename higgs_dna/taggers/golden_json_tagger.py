...
class GoldenJsonTagger(Tagger):
    ...
    """
    def __init__(self, name = "golden_json_tagger", options = {}, is_data = True, year = None,output_dir=None):
        super(GoldenJsonTagger, self).__init__(name, options, is_data, year,output_dir)
...
# from higgs_dna.utils import misc_utils
# @numba.njit
# def get_golden_json_cut(self, events):
#     # If no valid golden json file, return all True
#     if self.json_file is None:
#         cut = awkward.ones_like(events.run, dtype=bool)#abs(events.run) >= 0
#     else:
...
#     def calculate_selection(self, events):
#         cut = get_golden_json_cut(self, events)
#         self.register_cuts(
#                 names = ["golden json"],
#                 results = [cut]
#         )
