...
def select_nonisomuons(muons, options, clean, name = "none", tagger = None):
    """
    ...
    if options["global"]==True:
        ...
        global_cut = muons.pt > 0
    tuneptcut=muons.tunepRelPt*muons.pt>options["Tunept"]
    all_cuts = standard_cuts & tuneptcut & id_cut & global_cut 
    if tagger is not None:
        tagger.register_cuts(
                names = ["standard object cuts", "id cut", "global_muon cut", "all cuts"],
                results = [standard_cuts, id_cut, global_cut, all_cuts],
